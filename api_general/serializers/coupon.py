...
class CUCouponSerializer(ModelSerializer):
    hotel_ids = serializers.ListField(allow_null=True)
    tour_ids = serializers.ListField(allow_null=True)
    ...
    def validate(self, attrs):
        ...
        if not for_all and any([hotel_ids, tour_ids]):
            raise ValidationError("Hotel ids or tour ids must not be null in case for_all is false")
        if start_date > end_date:
            raise ValidationError("start date must be less than or equal end date")
    ...
    def update(self, instance, validated_data):
        ...
        CouponService.update(instance, coupon_data)
