...
def initial_hotel_and_review_data(apps, schema_editor):
    ...
    anonymous_partners: List[Profile] = ProfileService.bulk_create_anonymous_partners()
    ...
    for _file_name in hotel_data_files:
        with open(f"{hotel_data_directory}/{_file_name}", "r", encoding='utf-8') as file:
            ...
            hotel.save()
            saving_hotel_reviews(hotel_review_model, hotel, review_contents, anonymous_users)
...
def saving_hotel_reviews(hotel_review_model: Type[HotelReview], hotel: Hotel, review_contents: List[dict], anonymous_users: List[Profile]):
    ...
    for _review in review_contents:
        ...
        if reviewer_location == "dumpxxx":
            reviewer: Profile = random.choice(anonymous_users)
        else:
            reviewer: Profile = ProfileService.create_dump_user_with_name(reviewer_name)
