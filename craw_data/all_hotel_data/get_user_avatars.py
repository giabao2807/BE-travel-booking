...
def get_avatars(page_url):
    ...
    try:
        ...
        avatar_boxes = soup.find_all("a", {"class": "kjIqZ I ui_social_avatar inline"})
        for _avatar_box in avatar_boxes:
            avatar_url = _avatar_box.contents[0]["src"]
            if avatar_url:
                avatar_url = avatar_url.replace("photo-t", "photo-o")
...
for idx, file_name in enumerate(all_page):
    ...
    if file_name.endswith(".json"):
        ...
        _avatars = get_avatars(url)
        avatars.extend(_avatars)
        print(len(avatars))
